    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configure global referral bonuses and withdrawal limits</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <Label htmlFor="referralBonus">User Referral Bonus (₦)</Label>
              <Input
                id="referralBonus"
                type="number"
              ...
              <p className="text-xs text-muted-foreground">
                One-time bonus for referring a new user.
              </p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="minCommissionWithdrawal">Min Commission Withdrawal (₦)</Label>
              <Input
                id="minCommissionWithdrawal"
                type="number"
                value={settings.minCommissionWithdrawal}
                onChange={(e) => setSettings({ ...settings, minCommissionWithdrawal: Number(e.target.value) })}
              />
              <p className="text-xs text-muted-foreground">
                Minimum wallet balance for withdrawal.
              </p>
            </div>
            
             <div className="space-y-2">
              <Label htmlFor="agentCommission">Global Fallback Commission (₦)</Label>
              <Input
                id="agentCommission"
                type="number"
                value={settings.agentCommission}
                onChange={(e) => setSettings({ ...settings, agentCommission: Number(e.target.value) })}
              />
              <p className="text-xs text-muted-foreground">
                Used if no service-specific commission is set.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Transfer Service Charges</CardTitle>
          </div>
          <CardDescription>Configure fees charged to users for bank transfers</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setSettings({ ...settings, transferFee: Number(e.target.value) })}
              />
              <p className="text-xs text-muted-foreground">
                Flat fee charged per transfer transaction.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                id="stampDuty"
                type="number"
                value={settings.stampDuty || 0}
                onChange={(e) => setSettings({ ...settings, stampDuty: Number(e.target.value) })}
              />
              <p className="text-xs text-muted-foreground">
                Government stamp duty charge.
              </p>
            </div>
          </div>
        </CardContent>
