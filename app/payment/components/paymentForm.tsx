...
export default function PaymentProviderForm({
  ...
  const loading = useSelector(
    (state: RootState) => state.paymentProviders.loading
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (initialData?._id) {
        await dispatch(
          updatePaymentProvider({ id: initialData._id, data: payload })
        ).unwrap();
        toast.toast({
          title: "success",
        ...
        toast.toast({
          title: "success",
          description: " You can now use this provider for payment processing.",
        });
        onClose();
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure your payment API integration settings
          </CardDescription>
        </CardHeader>
