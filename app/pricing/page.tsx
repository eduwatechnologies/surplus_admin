...
export default function PricingPage() {
  ...
  const [serviceType, setServiceType] = useState<"data" | "airtime" | "electricity" | "cabletv" | "exampin">("data");
  const [network, setNetwork] = useState<"mtn" | "airtel" | "glo" | "9mobile" | "">("");
  ...
  const loadPlans = async () => {
    if (serviceType !== "data") {
      toast.toast({
        title: "Not available",
        description: "This simplified pricing screen is currently available for data plans only.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      for (const p of rows) {
        const current = typeof p.sellingPrice === "number" ? p.sellingPrice : (typeof p.basePrice === "number" ? p.basePrice : null);
  ...
  useEffect(() => {
    ...
    loadPlans();
  }, [user, isMerchant, serviceType, network, category, scope, selectedUserId]);
  ...
  const savePlanPrice = async (plan: CatalogPlan) => {
    ...
    if (!Number.isFinite(n)) {
      toast.toast({ title: "Invalid price", description: "Enter a valid number", variant: "destructive" });
      return;
    }
    if (scope === "user" && !selectedUserId) {
      toast.toast({
      ...
      <SidebarInset>
        ...
        <main className="flex-1 space-y-6 p-6">
          <div>
            ...
            <p className="text-muted-foreground">Service → Network → Plans. Set your selling price per plan.</p>
          </div>

          <Card>
            <CardHeader>
              <CardTitle>Pricing flow</CardTitle>
              <CardDescription>Pick what you sell, then quickly update plan prices.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-sm text-muted-foreground">
                  Step <span className="font-semibold text-foreground">{step}</span> of 3
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setStep((s) => (s === 1 ? 1 : ((s - 1) as any)))} disabled={step === 1}>
                    Back
                  </Button>
                  {step < 3 ? (
                    <Button
                      onClick={() => setStep((s) => (s === 1 ? 2 : 3))}
                      disabled={(step === 1 && serviceType !== "data") || (step === 2 && !network)}
                    >
                      Continue
                    </Button>
                  ) : null}
                ...
                <div className="space-y-3">
                  <div className="flex items-end justify-between gap-3">
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">Pick a network to load plans.</div>
                    </div>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="h-9 w-14 overflow-hidden rounded-md border bg-white">
                              <img src={`/images/${n.key}.svg`} alt={n.title} className="h-full w-full object-cover" />
                            </div>
                            <div>
              ...
              {step === 3 ? (
                <div className="space-y-6">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div className="text-sm text-muted-foreground">
                      Editing <span className="font-semibold text-foreground">{String(network).toUpperCase()}</span> /{" "}
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input value={filterText} onChange={(e) => setFilterText(e.target.value)} placeholder="e.g. 500MB, 1GB, 30 days" />
                    </div>
                    <div className="flex items-end gap-2">
                      <Button
                        variant="outline"
                        onClick={loadPlans}
                        disabled={loading || !network || !category.trim() || (scope === "user" && !selectedUserId)}
                      >
                          ...
                          filteredPlans.map((p) => {
                            ...
                            return (
                              <TableRow key={id}>
                                ...
                                <TableCell className="text-sm text-muted-foreground">{p.validity || "-"}</TableCell>
                                <TableCell className="text-sm">{base === null ? "-" : `₦${base}`}</TableCell>
                                <TableCell>
                                  <div className="flex flex-col gap-1">
                                    <Input
                                      value={edit}
                                      onChange={(e) => setPriceEdits((prev) => ({ ...prev, [id]: e.target.value }))}
