...
export default function ServicesPage() {
  ...
  const handleToggleServiceStatus = (serviceId: string) => {
    dispatch(toggleServiceStatus(serviceId));
  };
  ...
  const handleAddPlan = (args: {
    ...
    setEditingPlan(null);
    setPlanContext(args);
    setOpenDialogForPlan(args.subServiceId);
  };

  const handleEditPlan = (plan: any) => {
    setEditingPlan(plan);
    setPlanContext({
      subServiceId: plan.subServiceId,
      subCode: plan.subCode,
      provider: plan.provider,
      serviceType: plan.serviceType,
    });
    setOpenDialogForPlan(plan.subServiceId);
  };
                    ...
                    <TableRow key={service._id} className="hover:bg-muted/20">
                      ...
                      <TableCell>
                        <Switch
                          ...
                          onCheckedChange={() => handleToggleServiceStatus(service._id)}
                          disabled={isLoading}
                        />
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              onClick={() => setOpenDialogForService(service._id)}
                            >
                              Add Subservice
                            </DropdownMenuItem>
                          </DropdownMenuContent>
      ...
      >
        {openDialogForService && (
          <SubServiceForm
            ...
            initialValues={editingSubservice ?? undefined}
            onSubmitSuccess={() => {
              setOpenDialogForService(null);
              setEditingSubservice(null);
            }}
      ...
      >
        {openDialogForPlan && (
          <ServicePlanForm
            ...
            serviceType={planContext?.serviceType}
            subCode={planContext?.subCode}
            provider={planContext?.provider}
            onSubmitSuccess={() => {
              setOpenDialogForPlan(null);
              setEditingPlan(null);
              setPlanContext(null);
